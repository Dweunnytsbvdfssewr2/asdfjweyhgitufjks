...
do
	Library = {
		Open = true;
		Accent = Color3.fromRGB(101, 174, 197);
		Pages = {};
		Sections = {};
		Flags = {};
		UnNamedFlags = 0;
	...
	do
		...
		function Library:Window(Options)
			...
			Outline.Size = UDim2.new(0, 650, 0, 600)
			...
			function Window:KeyList() -- finobe wanted me to paste off eclipse incase this ever gets leaked LOL
				...
				--
				function NKeyList:NewKey(Name, Key, Mode)
					local KeyValue = {}
					--
					local NewKey = Instance.new("TextLabel")
					NewKey.Name = "NewKey"
					NewKey.FontFace = realfont
					NewKey.Text = Name .. " (" .. Mode .. "): " .. Key
					...
					function KeyValue:UpdateText(text)
						NewKey.Text = text
					end;
					return KeyValue
				end;
		...
		function Sections:Toggle(Properties)
			...
			function Toggle:Keybind(Properties)
				...
				if not Keybind.Ignore then
					ListValue = Library.KeyList:NewKey(Keybind.Name, Keybind.State, Keybind.Mode)
				end
				-- // Functions
				local function set(newkey)
					if string.find(tostring(newkey), "Enum") then
						...
						elseif newkey ~= nil then
							...
							ListValue:UpdateText(tostring(Keybind.Name.." ["..(Library.Keys[newkey] or tostring(newkey):gsub("Enum.KeyCode.", "")).."]"))
		...
		function Sections:Keybind(Properties)
			...
			--
			if not Keybind.Ignore then
				ListValue = Library.KeyList:NewKey(Keybind.Name, Keybind.Mode)
			end
			--
			Library.Flags[Keybind.Flag .. "_KEY"] = Keybind.State
			Library.Flags[Keybind.Flag .. "_KEY STATE"] = Keybind.Mode
