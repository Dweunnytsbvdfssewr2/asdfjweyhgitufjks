...
--
do -- checks
    if not isfolder("fluent/") then
		makefolder("fluent/");
	end
    --
	if not isfolder("fluent/Configs/") then
		makefolder("fluent/Configs/");
	end
	--
	if not isfolder("fluent/Luas/") then
		makefolder("fluent/Luas/");
	end
end
--
do -- macros
...
local fluent = {
    fonts = { 
        cache = {}, 
        options = { Graph_35 = {}, Smallest_Pixel = {}, Templeos = {}, ProggyTiny = {} },
    },
...
-- assignments
local workspace      = environment.workspace;
local players        = environment.players;
local stats          = environment.stats;
local lighting       = environment.lighting;
...
local pid_name       = environment.MarketplaceService:GetProductInfo(game.PlaceId).Name;
...
local library = ({
    open = true,
    font_size = 6,
    font = fluent.fonts.options.Templeos,
    ...
    menu_colors = {
        ...
        inline_accent = Color3.fromRGB(255, 110, 0), -- 71,80,255
        shadow_accent = Color3.fromRGB(255, 110, 0),
    };
...
do -- menu functions
    ...
    function library:resize(object, background)
        ...
        library:connection(get_mouse.Move, function()
            if dragging then
                local X = math.clamp(environment.user_input_service:GetMouseLocation().X - background.AbsolutePosition.X, 590, 1180)
                local Y = math.clamp((environment.user_input_service:GetMouseLocation().Y - 36) - background.AbsolutePosition.Y, 470, 940)
                currentsize = UDim2.new(0, X, 0, Y)
                background.Size = currentsize
            end
        end)
        
        library:connection(environment.user_input_service.InputEnded, function(input)
...
do -- menu elements
    function library:notification(message, duration, color, player)
            ...
            if index then
                for _, v in ipairs(notification.Objects) do
                    if v:IsA("Frame") or v:IsA("ImageLabel") then
                        environment.tween_service:Create(v, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
                    elseif v:IsA("TextLabel") then
                        environment.tween_service:Create(v, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 1}):Play()
                    end
                end
                ...
                for i = index, #library.notifications do
                    ...
                    environment.tween_service:Create(notif.Container, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = newPosition}):Play()
                end
            end
        end
        ...
        task.spawn(function()
            ...
            for _, v in next, notification.Objects do
                if v:IsA("Frame") then
                    environment.tween_service:Create(v, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
                elseif v:IsA("ImageLabel") then
                    environment.tween_service:Create(v, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
                end
            end
            environment.tween_service:Create(outline, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {AnchorPoint = Vector2.new(0, 0)}):Play()
            environment.tween_service:Create(value, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
            environment.tween_service:Create(player_image, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {ImageTransparency = 0}):Play()
            environment.tween_service:Create(time_line, TweenInfo.new(duration, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, 0)}):Play()
    ...
    function library:NewPicker(name, default, defaultalpha, parent, count, flag, callback)
        ...
        local function update()
            local real_pos = environment.UserInputService:GetMouseLocation()
        ...
        library:connection(environment.UserInputService.InputChanged, function(input)
        ...
        library:connection(environment.UserInputService.InputBegan, function(Input)
    ...
    function library:window(options)
        ...
        library:connection(environment.user_input_service.InputChanged, function(input)
            if window.dragging[1] and input.UserInputType == Enum.UserInputType.MouseMovement then
                local mouse_pos = environment.user_input_service:GetMouseLocation()
        ...
        function page:Turn(bool)
            ...
            for _, v in pairs(library.screen_gui:GetDescendants()) do
                if table.find({"Tab", "SectionOutline", "SectionInline", "Toggle_Outline", "Toggle_Inline", "Keybind_Outline", "Keybind_Inline", "Slider_Accent", "Slider_Outline", "Slider_Inline", "Toggle_Accent", "Button_Outline", "Button_Inline", "List_ContentOutline", "List_ContentInline", "List_Outline", "List_Inline", "ModeBox", "Icon_Line", "ContentOutline", "ContentInline", "Textbox_Outline", "Textbox_Inline", "Icon_Colorpicker"}, v.Name) then
                    ...
                    table.insert(tweens, environment.tween_service:Create(v, TweenInfo.new(0.4, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), { BackgroundTransparency = bool and 0 or 1 }))
                end
    
                if v:IsA("TextLabel") and v.Name ~= "Window_Name" then
                    if bool then v.TextTransparency = 1 end
                    table.insert(tweens, environment.tween_service:Create(v, TweenInfo.new(0.4, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), { TextTransparency = bool and 0 or 1 }))
                end        
            end
            ...
            environment.tween_service:Create(tab_button, TweenInfo.new(0.4, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {
            ...
            environment.tween_service:Create(button_stroke, TweenInfo.new(0.3, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {
    ...
    function sections:toggle(properties)
        ...
        local function set_state()
            ...
            if Toggle.Risk then
                environment.tween_service:Create(Accent, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundTransparency = Toggle.Toggled and 0 or 1}):Play()
                environment.tween_service:Create(Title, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {TextColor3 = not Toggle.Toggled and Color3.fromRGB(75, 0, 0) or Color3.fromRGB(255, 0, 0)}):Play()
            else
                environment.tween_service:Create(Accent, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundTransparency = Toggle.Toggled and 0 or 1}):Play()
                environment.tween_service:Create(Title, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {TextColor3 = not Toggle.Toggled and Color3.fromRGB(125, 125, 125) or Color3.fromRGB(255, 255, 255)}):Play()
            end
        ...
        function Toggle:keybind(Properties)
            ...
            KInline.MouseButton1Click:Connect(function()
                if not Keybind.Binding then
                    ...
                    Keybind.Binding = library:connection(
                        environment.user_input_service.InputBegan,
                        function(input, gpe)
            ...
            library:connection(environment.user_input_service.InputBegan, function(inp)
                if (inp.KeyCode == Key or inp.UserInputType == Key) and not Keybind.Binding and not Keybind.UseKey then
                    if Keybind.Mode == "Hold" then
                        ...
                        c = library:connection(environment.run_service.RenderStepped, LPH_NO_VIRTUALIZE(function()
            ...
            library:connection(environment.user_input_service.InputEnded, function(inp)
    ...
    function sections:slider(Properties)
        ...
        local function is_set(value)
            ...
            current_tween = environment.tween_service:Create( Accent, TweenInfo.new(0.13, Enum.EasingStyle.Linear), {Size = targetSize} )
            current_tween:Play()
    ...
    function sections:list(Properties)
        ...
        do -- properties
            ...
            Icon_Line.Position = UDim2.new(0, 162, 0, 0)
    ...
    function sections:keybind(Properties)
        ...
        Inline.MouseButton1Click:Connect(function()
            if not Keybind.Binding then
                ...
                Keybind.Binding = library:connection(
                    environment.UserInputService.InputBegan,
                    function(input, gpe)
                        set( input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode or input.UserInputType )
                        library:Disconnect(Keybind.Binding)
                        task.wait()
        ...
        --
        library:connection(environment.UserInputService.InputBegan, function(inp)
            if (inp.KeyCode == Key or inp.UserInputType == Key) and not Keybind.Binding and not Keybind.UseKey then
                if Keybind.Mode == "Hold" then
                    if Keybind.Flag then
                        library.flags[Keybind.Flag] = true
                    end
                    c = library:connection(environment.RunService.RenderStepped, LPH_NO_VIRTUALIZE(function()
        ...
        --
        library:connection(environment.UserInputService.InputEnded, function(inp)
            if Keybind.Mode == "Hold" and not Keybind.UseKey then
    ...
    --
    do -- fade out & blur
        local blur_effect = cloneref(Instance.new("BlurEffect", lighting))
        ...
        --environment.user_input_service.MouseIconEnabled = not library.open
        
        library:connection(environment.user_input_service.InputBegan, function(input)
            if input.KeyCode == library.ui_key and not is_fading then
                ...
                for _, v in pairs(library.screen_gui:GetDescendants()) do
                    if v.Name == "Outline_Background" or v.Name == "Background" or v.Name == "Inline_Background" or v.Name == "SectionOutline" or v.Name == "SectionInline" or v.Name == "Toggle_Outline" or v.Name == "Toggle_Inline" or v.Name == "Keybind_Outline" or v.Name == "Keybind_Inline" or v.Name == "Slider_Accent" or v.Name == "Slider_Outline" or v.Name == "Slider_Inline" or v.Name == "Toggle_Accent" or v.Name == "Button_Outline" or v.Name == "Button_Inline" or v.Name == "List_ContentOutline" or v.Name == "List_ContentInline" or v.Name == "List_Outline" or v.Name == "List_Inline" or v.Name == "ModeBox" or v.Name == "Icon_Line" or v.Name == "Tab" or v.Name == "ContentOutline" or v.Name == "ContentInline" or v.Name == "Textbox_Outline" or v.Name == "Textbox_Inline" or v.Name == "Icon_Colorpicker" then
                        table.insert(tweens, environment.tween_service:Create(v, TweenInfo.new(0.33, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { BackgroundTransparency = library.open and 1 or 0 }))
                    end
    
                    if v:IsA("UIStroke") then
                        table.insert(tweens, environment.tween_service:Create(v, TweenInfo.new(0.33, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { Transparency = library.open and 1 or 0 }))
                    end
                    if v:IsA("ImageLabel") then 
                        table.insert(tweens, environment.tween_service:Create(v, TweenInfo.new(0.33, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { ImageTransparency = library.open and 1 or 0 }))
                    end
                    if v:IsA("TextLabel") or v:IsA("TextButton") then
                        table.insert(tweens, environment.tween_service:Create(v, TweenInfo.new(0.33, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { TextTransparency = library.open and 1 or 0 }))
                    end
                end
                
                local blackbg_tween = environment.tween_service:Create(black_bg, TweenInfo.new(0.33, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { BackgroundTransparency = library.open and 1 or 0.5 })
                table.insert(tweens, blackbg_tween)
                
                local blur_tween = environment.tween_service:Create(blur_effect, TweenInfo.new(0.33, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { Size = flags["blur_filter"] and not library.open and 20 or 0 })
    ...
    -- local menu = library:window({name = 'fluent', position = "left"});
    -- local watermark = library:watermark({ Name = 'fluent | build: beta | fps: 0' });
    ...
    --             local List = {}
    --             for _, file in ipairs(listfiles("fluent/Configs")) do 
    --                 List[#List + 1] = file:match("fluent/Configs\\(.*)%.cfg")
    --             end
    ...
    --                 if configName ~= "" and not isfile("fluent/Configs/" .. configName .. ".cfg") then
    --                     writefile("fluent/Configs/" .. configName .. ".cfg", library:GetConfig())
    ...
    --                     writefile("fluent/Configs/" .. selectedConfig .. ".cfg", library:GetConfig())
    ...
    --                 local selectedConfig = flags.cfg_list
    --                 if selectedConfig then
    --                     library:LoadConfig(readfile("fluent/Configs/" .. selectedConfig .. ".cfg"))
    ...
    --                 local selectedConfig = flags.cfg_list
    --                 if selectedConfig then
    --                     delfile("fluent/Configs/" .. selectedConfig .. ".cfg")
